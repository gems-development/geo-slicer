...
public class BoundingHoleDeleter
{
        ...
        while (listOfHoles.First!.Next is not null)
        {
            ...
            if (!hasIntersectFrames)
            {
                ...
                else
                {
                    ...
                    {
                        BruteforceConnectWithIntersectRing(thisRing, listOfHoles, cache);
                    }
            ...
            {
                BruteforceConnectIntersectionBoundRFrames(thisRing, listOfHoles, cache);
                if (thisRing.Value.PointMin.Equals(pointMinShell) && thisRing.Value.PointMax.Equals(pointMaxShell))
                {
                    var buff = thisRing.Value;
                    listOfHoles.Remove(thisRing);
    ...
    private void BruteforceConnectIntersectionBoundRFrames(
        ...
        PartitionBoundRingsCache cache)
    {
        ...
        do
        {
            ...
            var intersectOfFrames = BoundRIntersectsChecker.GetIntersectionBoundRFrames(thisRing.Value, currentFrame.Value);
            do
            {
                if (!flagFirstCycle)
                {
                ...
                if (!flagSecondCycle)
                {
                    ...
                    do
                    {
                        if (BoundRIntersectsChecker.PointInsideFrameCheck(startCurrentFrame.Elem, intersectOfFrames))
                        {
                ...
                {
                    if (BoundRIntersectsChecker.IntersectBoundRingNotExtPoints(thisRing, startThisRing.Elem, startCurrentFrame.Elem))
                    {
                        flagFirstCycle = false;
                        startThisRing = startThisRing.Next;
                    }

                    if (BoundRIntersectsChecker.IntersectBoundRingNotExtPoints(currentFrameNode.Value, startThisRing.Elem,
                    ...
                    if (flagFirstCycle && flagSecondCycle)
                    {
                        foreach (var frame in cache.IntersectFrames)
                        {
                            if (!ReferenceEquals(currentFrameNode.Value, frame))
                            {
                                ...
                                {
                                    if (BoundRIntersectsChecker.IntersectBoundRing(frame, startThisRing.Elem, startCurrentFrame.Elem))
                                    {
                                        flagFirstCycle = false;
                                        flagSecondCycle = false;
                                        startThisRing = startThisRing.Next;
    ...
    private void BruteforceConnectWithIntersectRing(LinkedListNode<BoundingRing> thisRing, LinkedList<BoundingRing> listOfHoles, PartitionBoundRingsCache cache)
    {
